...
void AEX_DOL_Patch_SpecialDOLData( UCHAR lenOfT, UCHAR *datOfTag )
{
	UINT  iTag;
	UCHAR i;
	UCHAR buf1[22];
...
UCHAR AEX_DOL_Patch_DOLData(UINT iptLen, UCHAR *iptDOL, UCHAR *optLen, UCHAR *optData)
{
	UCHAR	lenOfT=0;
	UCHAR	lenOfL=0;
	UINT	lenOfV=0;
...
//		if (apk_EMVCL_CheckConsTag(iptDOL[0]) == TRUE)
		...
		if (flgDntSearch == FALSE)
		{
			...
			if (rspCode == SUCCESS)
			{
				if (glv_tagTable[recIndex].AE_IsKnown == AE_KNOWN)
				{
					AEX_DOL_Patch_SpecialDOLData(lenOfT, iptDOL);
...
UCHAR AEX_DOL_Get_DOLData(UINT iptLen, UCHAR *iptDOL, UCHAR *optLen, UCHAR *optData)
{
	UCHAR	tmpLen=0;
	...
	rspCode=AEX_DOL_Patch_DOLData(iptLen, iptDOL, &tmpLen, tmpBuffer);
	if (rspCode == SUCCESS)
	{
