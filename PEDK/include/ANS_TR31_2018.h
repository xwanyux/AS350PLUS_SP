...
//**  PRODUCT  : AS350-X6							                        **
//**                                                                        **
//**  FILE     : ANS_TR31_2018.H					                        **
//**  MODULE   : Declaration of ANSI TR-31 Module for AES.		            **
//**  VERSION  : V1.00                                                      **
//**  DATE     : 2022/05/24                                                 **
//**  EDITOR   : James Hsieh                                                **
//**                                                                        **
...
//
//----------------------------------------------------------------------------
#ifndef _ANS_TR31_2018_H_
#define _ANS_TR31_2018_H_
...
////----------------------------------------------------------------------------
#define	KEY_BUNDLE_LEN2		112			// AES, whole key bundle
#define	KEY_DATA_LEN2		32			// AES, KeyLen(2) + Key(16) + Padding(6+8)
//
...
typedef	struct	PED_KEY_BUNDLE2_S			// ASCII format (AES)
{
	UINT8			VersionID[1];		// key block version id
	UINT8			BlockLen[4];		// key block length
	UINT8			Usage[2];		// key usage
	...
	UINT8			Padding[28];		// random padding
	UINT8			MAC[32];		// MAC values after truncation	(TR-31 2018)
	
} PED_KEY_BUNDLE2;

typedef	struct	PED_KEY_DATA2_S				// HEX format (AES)
{	
	UINT8			KeyLen[2];		// key length
	UINT8			Key[16];		// key data
	UINT8			Padding[14];		// random padding
	UINT8			MAC[16];		// MAC values after truncation	(TR-31 2018)
...
//	Function Prototypes
//----------------------------------------------------------------------------
extern	UINT32	TR31_VerifyKeyBundle_AES( UINT16 Length, UINT8 *KeyBundle, UINT8 *eKeyData, UINT8 *mac );
extern	UINT32	TR31_DecryptKeyBundle_AES( UINT8 *icv, UINT8 *eKeyData, UINT8 *key );
