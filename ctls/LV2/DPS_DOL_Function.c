...
//D-PAS MS Mode
extern UCHAR MS_Mode;	

void DPS_DOL_Patch_SpecialDOLData( UCHAR lenOfT, UCHAR *datOfTag )
{
	UINT  iTag;
	UCHAR i;
	UCHAR buf1[22];
...
*/
		case 0x9F37: // unpredictable number
			...
			{
				//UN should be 02~99
				for(i = 0 ; i < glv_tag9F37.Length[0] ; i++)
				{
					do
					{
						api_sys_random(&randomNum);
						data_l = randomNum & 0x0F;
						data_h = (randomNum & 0xF0) >> 4;
			
					} while((data_l < 0x02) || (data_l > 0x09) || (data_h > 0x09));

					glv_tag9F37.Value[i] = randomNum;
				}
			}
			else  //EMV mode
			{
...
UCHAR DPS_DOL_Patch_DOLData(UINT iptLen, UCHAR *iptDOL, UCHAR *optLen, UCHAR *optData)
{
	UCHAR	lenOfT=0;
	UCHAR	lenOfL=0;
	UINT	lenOfV=0;
...
//		if (apk_EMVCL_CheckConsTag(iptDOL[0]) == TRUE)
		...
		if (flgDntSearch == FALSE)
		{
			...
			if (rspCode == SUCCESS)
			{
				if(glv_tagTable[recIndex].DPS_IsKnown == DPS_KNOWN)
				{
					DPS_DOL_Patch_SpecialDOLData(lenOfT, iptDOL);
...
UCHAR DPS_DOL_Get_DOLData(UINT iptLen, UCHAR *iptDOL, UCHAR *optLen, UCHAR *optData)
{
	UCHAR	tmpLen=0;
	...
	rspCode=DPS_DOL_Patch_DOLData(iptLen, iptDOL, &tmpLen, tmpBuffer);
	if (rspCode == SUCCESS)
	{
